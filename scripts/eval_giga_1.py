...
from vgn.dataset_voxel1 import DatasetVoxelOccFile
...
def main(args):
    ...
    # create data loaders
    val_loader = create_train_val_loaders(
    ...
    logger = Logger1(root=args.dataset)

    net.eval()
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            x, y, pos, pos_occ, per_success, scene_id = prepare_batch(batch, device)
            # print("pos:", pos.shape)
            # print("pos_x:", min(pos[:,:,0]), max(pos[:,:,0]))
            # print("pos_y:", min(pos[:,:,1]), max(pos[:,:,1]))
            # print("pos_y:", min(pos[:,:,2]), max(pos[:,:,2]))
            y_pred = select(net(x, pos, p_tsdf=pos_occ))
            loss, loss_dict = loss_fn(y_pred, y)
            # print("loss_dict:", loss_dict['loss_occ'], "per_success:", per_success, "scene_id:", scene_id,"\n")
            logger.log_result(scene_id[0], per_success.cpu().numpy()[0], loss_dict['loss_occ'].cpu().numpy())
            # logger.log_result(scene_id, per_success, loss_dict['loss_occ'])

class Logger1(object):
    def __init__(self, root):

        self.root = root
        self.grasp_sucess_path = self.root / "jointly_optimize.csv"
        self._create_csv_files_if_needed()

    def _create_csv_files_if_needed(self):

        if not self.grasp_sucess_path.exists():
            columns = [
                "scene_id",
                "per_success",
                "loss_occ"
            ]
            io.create_csv(self.grasp_sucess_path, columns)

    def log_result(self, scene_id, per_success, loss_occ):
        io.append_csv(
            self.grasp_sucess_path,
            scene_id,
            per_success,
            loss_occ,
        )
...
def prepare_batch(batch, device):
    pc, (label, rotations, width), pos, pos_occ, occ_value, per_success, scene_id = batch
    ...
    return pc, (label, rotations, width, occ_value), pos, pos_occ, per_success, scene_id
