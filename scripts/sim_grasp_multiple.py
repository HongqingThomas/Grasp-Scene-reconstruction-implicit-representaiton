...
from vgn.incremental_implicit import Incremental_VGNImplicit
from vgn.experiments import clutter_removal, clutter_removal_1
...
def main(args):
    ...
    net = load_network(args.model, device, model_type=args.type)
    # net = get_network(args.type).to(device)
    incremental_plannar = Incremental_VGNImplicit(args.model,
                                    args.type,
                                    best=args.best,
    ...
    if args.type in ['giga', 'giga_aff', 'giga_hyper']:
        grasp_planner = VGNImplicit(args.model, #TODO: new: 这个地方的model是parameter path, 应该修改为incremental plannar的输出
                                    args.type,
                                    best=args.best,
                                    qual_th=args.qual_th,
                                    force_detection=args.force,
    ...
    for seed in args.seeds:
        ...
        success_rate, declutter_rate = clutter_removal_1.run(
            network = net,
            incremental_plannar = incremental_plannar,
