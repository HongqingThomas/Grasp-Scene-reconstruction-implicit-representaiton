...
class ClutterRemovalSim(object):
    def __init__(self, scene, object_set, gui=True, seed=None, add_noise=False, sideview=False, save_dir=None, save_freq=8):
        ...
        self.fx, self.fy, self.cx, self.cy = 540.0, 540.0, 320.0, 240.0
        intrinsic = CameraIntrinsic(640, 480, self.fx, self.fy, self.cx, self.cy)
    ...
    def acquire_tsdf(self, n, N=None, resolution=40):
        ...
        # extrinsic_for_render = 

        timing = 0.0

        depth_img_list, intrinsic_list, extrinsic_list = [], [], []
        intrinsic_matrix = [[self.fx, 0, self.cx], 
                            [0, self.fy, self.cy],
                            [0, 0, 1]]
        for extrinsic in extrinsics:
            ...
            extrinsic_list.append(extrinsic.as_matrix())
        bounding_box = o3d.geometry.AxisAlignedBoundingBox(self.lower, self.upper)
        pc = high_res_tsdf.get_cloud()
        pc = pc.crop(bounding_box)
        return tsdf, pc, timing, depth_img_list, intrinsic_list, extrinsic_list
    ...
    def calculate_extrinsic_render_try(self, origin, radius, theta, phi_list):
        for phi in phi_list:
            translation_m = np.r_[
