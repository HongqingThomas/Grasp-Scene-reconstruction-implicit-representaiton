...
from vgn.networks import load_network, get_network
...
class Incremental_VGNImplicit(object):
    def __init__(self, model_path, model_type, best=False, force_detection=False, qual_th=0.9, out_th=0.5, visualize=False, resolution=40, **kwargs):
        ...
        self.net = get_network(model_type).to(self.device)
        ...
        self.incremental_iter_nums = 3 # 3

    def __call__(self, state,  depth_img_list=None, intrinsic_list=None, extrinsic_list=None, network = None, aff_kwargs={}):
        self.net = network
        ...
        # rendering part
        print("depth_image_list:", len(depth_img_list), depth_img_list[0].shape)
        ...
        sampling_size = self.sampling_number
        Hedge,Wedge = int(H/2 - 100), int(W/2 - 100)
        # print("extrinsic_matrix:", extrinsic_matrix)
        c2w = extrinsic_matrix
        c2w[1, :] *= -1
        c2w[0, :] *= -1
        self.bound = torch.from_numpy(np.array([[0+0.02, 0.3-0.02], [0+0.02, 0.3-0.02], [0+0.055, 0.3]])).to(self.device)
        for i in range(self.incremental_iter_nums):
            ...
            with torch.no_grad():
                ...
                # print("det_rays_o:", det_rays_o)
                # print("det_rays_d:", det_rays_d.shape)
                # has some error
            ...
            print("loss_vals:", batch_gt_depth-depth)
    ...
    def render_batch_ray(self, rays_d, rays_o, device, stage, gt_depth, tsdf_vol):
        ...
        # print("gt_depth:", gt_depth)clear

        gt_depth = gt_depth.reshape(-1, 1)
        gt_depth_samples = gt_depth.repeat(1, N_samples)
        near = gt_depth_samples*0.5
...
def predict(tsdf_vol, pos, pos_occ, net, device):
    ...
    # pos_occ: scale from [0, 0.3] -> [-0.5, 0.5]
    pos_occ = (pos_occ / 0.3) - 0.5
